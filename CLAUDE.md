...
exceptions and hash-only postState fixtures need them). Known cost: each
`point-eval` call re-parses the KZG trusted setup (~1s); only the
point-evaluation fixture file pays this.
...
- **Never run a test sample without first proving it runs ≤30s** (estimate
  ...
  sign-off. Known stragglers: `vmPerformance/`, `stTimeConsuming/`, the
  `DelayFiles` list in `Conform/Main.lean`.
