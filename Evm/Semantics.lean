...
open Evm.DebuggingAndProfiling
...
def byteAt (μ₀ μ₁ : UInt256) : UInt256 :=
  let v₁ : BitVec 256 := BitVec.ofNat 256 μ₁.toNat
  let vᵣ : BitVec 256 := BitVec.ofFn (λ i => if i >= 248 && μ₀ < 32
                                             then v₁.getLsbD i
                                             else false)
  Evm.UInt256.ofNat (BitVec.toNat vᵣ)
...
def call (fuel : Nat)
  ...
  (permission : Bool)
  (evmState : ExecutionState)
    :
  Except ExecutionException (UInt256 × ExecutionState)
:= do
  match fuel with
    | 0 => .error .OutOfFuel
    | .succ f =>
      ...
      let result : ExecutionState := { evmState with accountMap := σ', substate := A', createdAccounts := cA }
      let result := {
...
:=
  match fuel with
    ...
    | .succ f =>
    λ (evmState : ExecutionState) ↦ do
    -- This will normally be called from `Ξ` (or `X`) with `fetchInstr` already having been called.
    -- That said, we sometimes want a `step : Transformer` and as such, we can decode on demand.
    let (instr, arg) ←
      match instr with
    ...
    match instr with
        ...
        match evmState.stack.pop3 with
          | some ⟨stack, μ₀, μ₁, μ₂⟩ => do
            ...
            let (a, evmState', g', z, o)
                  : (AccountAddress × ExecutionState × UInt256 × Bool × ByteArray)
              :=
      ...
      | .CREATE2 =>
        ...
        match evmState.stack.pop4 with
          | some ⟨stack, μ₀, μ₁, μ₂, μ₃⟩ => do
            let i := evmState.memory.readWithPadding μ₁.toNat μ₂.toNat
            let ζ := Evm.UInt256.toByteArray μ₃
            let I := evmState.executionEnv
            let Iₐ := evmState.executionEnv.codeOwner
            let Iₒ := evmState.executionEnv.sender
            let Iₑ := evmState.executionEnv.depth
            ...
            let σ_Iₐ : Account := σ.find? Iₐ |>.getD default
            let σStar := σ.insert Iₐ {σ_Iₐ with nonce := σ_Iₐ.nonce + 1}
            let (a, evmState', g', z, o) : (AccountAddress × ExecutionState × UInt256 × Bool × ByteArray) :=
              if σ_Iₐ.nonce.toNat ≥ 2^64-1 then (default, evmState, .ofNat (L evmState.gasAvailable.toNat), False, .empty) else
              if μ₀ ≤ (σ.find? Iₐ |>.option 0 (·.balance)) ∧ Iₑ < 1024 ∧ i.size ≤ 49152 then
                let Λ :=
                  Lambda f
      ...
      | .STATICCALL =>
        ...
        let evmState' := state'.replaceStackAndIncrPC μ'ₛ
        .ok evmState'
      | instr => stepPrimop instr arg {evmState with gasAvailable := evmState.gasAvailable - UInt256.ofNat gasCost}

/--
  Iterative progression of `step`
-/
def X (fuel : ℕ) (validJumps : Array UInt256) (evmState : ExecutionState)
  : Except ExecutionException (ExecutionResult ExecutionState)
:= do
  match fuel with
    | 0 => .error .OutOfFuel
    | .succ f =>
...
:= do
  match fuel with
    | 0 => .error .OutOfFuel
    | .succ f =>
      let defState : ExecutionState := default
      let freshEvmState : ExecutionState :=
        { defState with
            accountMap := σ
            σ₀ := σ₀
            executionEnv := I
...
def Θ (fuel : Nat)
      ...
      (w  : Bool)
        :
      Except ExecutionException (Batteries.RBSet AccountAddress compare × AccountMap × UInt256 × Substate × Bool × ByteArray)
:=
  match fuel with
    | 0 => .error .OutOfFuel
    | fuel + 1 => do
